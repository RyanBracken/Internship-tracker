...
router.get('/', (req, res) => {
  const db = getDb();
  const {
    q, category, company, salary_type, source_type, source,
    page = 1, limit = 20, sort = 'date_posted', order = 'desc'
  } = req.query;
  ...
  const rows = db.prepare(`
    ...
    ${where}
    ORDER BY i.${sortCol} ${sortOrder} NULLS LAST
    LIMIT ? OFFSET ?
...
router.get('/stats', (req, res) => {
  ...
  const categories = db.prepare(`
    SELECT category, COUNT(*) as count FROM internships WHERE is_active=1 AND is_duplicate=0
    GROUP BY category ORDER BY count DESC
  `).all();
  const sources = db.prepare(`
    SELECT source, COUNT(*) as count FROM internships WHERE is_active=1 AND is_duplicate=0
    GROUP BY source ORDER BY count DESC
  `).all();
  const companies = db.prepare(`
    SELECT company, COUNT(*) as count FROM internships WHERE is_active=1 AND is_duplicate=0
    GROUP BY company ORDER BY count DESC LIMIT 20
  `).all();
  const lastScrape = db.prepare(`
    SELECT MAX(finished_at) as last_updated FROM scrape_log WHERE status='success'
  `).get();
  const total = db.prepare(`SELECT COUNT(*) as count FROM internships WHERE is_active=1 AND is_duplicate=0`).get();

  res.json({ total: total.count, categories, sources, companies, last_updated: lastScrape?.last_updated });
});
